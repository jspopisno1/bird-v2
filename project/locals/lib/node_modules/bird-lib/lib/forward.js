...
var forward = function(config, httpOrHttps, urlOptions, req, res) {
    ...
    var forwardRequest = httpOrHttps.request(urlOptions, function (forwardResponse) {
        ...
        if (forwardResponse.statusCode != 200) {
            console.log('@debug, ', forwardResponse.location, forwardResponse.statusCode)

            birdUtils.log('warn', 'Response status got is : ' + forwardResponse.statusCode)
...
module.exports = function (config, hitRouteInfo, server, req, res, next) {
    ...
    forward(config, httpOrHttps, urlOptions, req, res)
}